...
public class DashboardTutor extends JFrame implements MenuListener {
    ...
    public DashboardTutor() {
        ...
        currentPanel = register; // Mặc định
    ...
    public void onMenuSelected(String menuText) {
        ...
        int tutorId = SessionManager.getInstance().getUserId(); // Lấy tutorId từ session
        ...
        switch (menuText) {
            ...
            case "Lịch dạy":
                ...
                schedule.loadScheduleData(tutorId); // Truyền tutorId
                break;
            case "Thông tin cá nhân":
                currentPanel = profileTutor;
                profileTutor.loadProfileData(tutorId); // Truyền tutorId
                break;
            case "Thống kê lương":
            ...
            case "Đăng xuất":
                ...
                SessionManager.getInstance().setUserId(-1); // Xóa session
                dispose(); // Đóng DashboardTutor
                new LoginForm().setVisible(true); // Quay lại màn đăng nhập
                return;
        }
        add(currentPanel, BorderLayout.CENTER);
        revalidate();
